...
from src.utils.ConfirmationResize import ResizeConfirmationDialog
...
class ResizePdf(QMainWindow):
    def __init__(self):
        ...
        for fmt in formats:
            ...
            button.clicked.connect(lambda checked, f=fmt: self.ask_for_resize(f))
        ...
        Resize300x500Button.clicked.connect(lambda: self.ask_for_resize("300x500"))
        custom_button_layout.addWidget(Resize300x500Button)

        CustomSizeButton = self.create_button("Custom")
        CustomSizeButton.clicked.connect(self.show_custom_size_dialog)
